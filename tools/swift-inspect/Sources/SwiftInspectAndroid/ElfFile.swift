...
public class ElfFile {
  ...
  typealias SymbolMap = [String: (start: UInt64, end: UInt64)]
  ...
  func readShdr<T: ElfShdr>(index: UInt) throws -> T {
  ...
  func loadSymbols(baseAddress: UInt64 = 0) throws -> SymbolMap {
    ...
    for sectionIndex in 0..<sectionCount {
      ...
      guard let linkIndex = UInt(exactly: shdr.link) else {
        throw Error.MalformedElfFile(
          self.filePath, description: "invalid shrr.link: \(shdr.link)")
      }

      let shdrLink: ElfShdr = isElf64
